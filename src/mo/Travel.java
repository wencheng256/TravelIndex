...
public class Travel implements Sqlable {
	...
	public boolean insert() {
		...
		if(name==null||tip==null||price==0||image==null||content==null||label==null||city==0||seller==0)
			return false;
		String sql="INSERT INTO travel(name,tip,price,image,content,label,city,seller) values('"+name+"','"+tip+"',"+price+",'"+image+"','"+content+"','"+label+"',"+city+","+seller+")";
	...
	public int update() {
		...
		if(id==0)
			return -1;
		  String sql="UPDATE travel SET name='"+name+"',tip='"+tip+"',price="+price+",image='"+image+"',content='"+content+"',label='"+label+"',city="+city+",seller="+seller+" where id="+id;
	...
	public Iterator<Travel> selectAll() {
		// TODO 自动生成的方法存根
		String sql="select a.id,a.name,a.tip,a.price,a.image,a.content,a.label,a.city,a.seller,b.name,c.name from travel as a,city as b,seller as c where a.city=b.id and a.seller=c.id";
		...
		try {
			while(rs.next())
			{
				...
				travel.setCityname(rs.getString(10));
				travel.setSellername(rs.getString(11));
	...
	public Iterator<Travel> selectAll(int start,int size) {
		// TODO 自动生成的方法存根
		String sql="select a.id,a.name,a.tip,a.price,a.image,a.content,a.label,a.city,a.seller,b.name,c.name from travel as a,city as b,seller as c where a.city=b.id and a.seller=c.id limit "+start+","+size;
		ResultSet rs=mysql.query(sql);
		LinkedList<Travel> list=new LinkedList<Travel>();	
		try {
			while(rs.next())
	...
	public Travel selectedId(int id) {
		// TODO 自动生成的方法存根
		String sql="select a.id,a.name,a.tip,a.price,a.image,a.content,a.label,a.city,a.seller,b.name,c.name from travel as a,city as b,seller as c where a.city=b.id and a.seller=c.id and a.id="+id;
		ResultSet rs=mysql.query(sql);
		try {
			if(rs.next())
			{
	...
	public int getCount()
	{
		String sql="select count(id)from travel";
		ResultSet rs=mysql.query(sql);
		try {
			if(rs.next())
			{			
	...
	// TODO 自动生成的方法存根
		Travel test=new Travel(1,"111","222",2,"333","444","555",8,4);
