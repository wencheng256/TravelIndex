...
//Getter and Setter	
	...
	public Iterator<Adv> selectAll(int start,int size) {
		// TODO 自动生成的方法存根
		String sql="select id,name,image,loc,link from adv limit "+start+","+size;
		ResultSet rs=mysql.query(sql);
		LinkedList<Adv> list=new LinkedList<Adv>();	
		try {
			while(rs.next())
	...
	public  Iterator<Adv> selectname(String name){
		
		
		String sql="select * from adv where name like '"+name+"%'";
		ResultSet rs=mysql.query(sql);
		LinkedList<Adv> list=new LinkedList<Adv>();	
	...
	public int getCount()
	{
		String sql="select count(id)from adv";
		ResultSet rs=mysql.query(sql);
		try {
			if(rs.next())
			{			
