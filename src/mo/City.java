...
public class City implements Sqlable {
	...
	public Iterator<City> selectAll() {
		// TODO 自动生成的方法存根
		String sql="select a.id,a.name,a.province,b.name from city as a,province as b where b.id=a.province";
	...
	public Iterator<City> selectAll(int start,int size) {
		// TODO 自动生成的方法存根
		String sql="select a.id,a.name,a.province,b.name from city as a,province as b where b.id=a.province limit "+start+","+size;
		ResultSet rs=mysql.query(sql);
		LinkedList<City> list=new LinkedList<City>();	
		try {
			while(rs.next())
			{
				...
				city.setName(rs.getString(2));
				city.setProvince(rs.getInt(3));
				city.setProName(rs.getString(4));
	...
	public City selectedId(int id) {
		// TODO 自动生成的方法存根
		String sql="select a.id,a.name,a.province,b.name from city as a,province as b where b.id=a.province and a.id="+id;
		ResultSet rs=mysql.query(sql);
		try {
			if(rs.next())
			{
