...
public class TicketLabel implements Sqlable {
	...
	public Iterator<TicketLabel> selectAll(int start,int size) {
		String sql="select id,name from ticketlabel limit "+start+","+size;
		ResultSet rs=mysql.query(sql);
		LinkedList<TicketLabel> list=new LinkedList<TicketLabel>();	
		try {
			while(rs.next())
	...
	public int getCount()
	{
		String sql="select count(id) from ticketLabel";
		ResultSet rs=mysql.query(sql);
		try {
			if(rs.next())
			{			
